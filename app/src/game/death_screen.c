...
void ui_death_screen(tenv* env) {
  ...
  if (igBegin("##death_screen", NULL, flags)) {
    ...
    if (screenshot_run_count() > 0) {
      igDummy((ImVec2){card_w, 16.0f});
      char review_label[48];
      snprintf(review_label, sizeof(review_label),
              "\uea1a Kill Shots (%d)##death_screen",
              screenshot_run_count());
      if (igButton(review_label, (ImVec2){card_w, btn_h})) {
        /* Same disconnect path as Lobby -- loop.c checks this flag right
           after the reset that path triggers and routes to the review
           screen instead of the title screen. */
        gdata->death_pending = false;
        gdata->kill_review_pending = true;
        if (gdata->connection) gdata->connection->is_closing = true;
      }
    }
  }
