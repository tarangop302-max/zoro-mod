...
void input(tenv* env) {
  ...
  if (gdata->data.follow_view) {
    ...
    float ang;
    /* Human turning input: send the new heading as soon as it changes,
       capped at a ~120Hz send rate (8ms) instead of the old 50ms gate.
       That's well below what's perceptible, but still bounds how often
       mg_ws_send() can be hit -- with the old gate removed entirely,
       an uncapped/high frame rate could call this hundreds of times a
       second and flood the socket. sang-change dedupe below still
       avoids redundant sends on top of this. Bot-controlled turning is
       untouched: it goes through this same path already. */
    if (want_e && gdata->data.ctm - gdata->data.last_e_mtm > 8) {
      want_e = false;
